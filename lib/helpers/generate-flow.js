...
module.exports = function (generator, options, runtimeOpts) {
	...
	var registerTemplateHelpers = function (helpers) {
		for (var helper in helpers) {
			if (helpers.hasOwnProperty(helper)) {
				Handlebars.registerHelper(helper, helpers[helper]);
			}
		}
		// Register the layout engine helpers
		Handlebars.registerHelper(handlebarsLayouts(Handlebars));
	};

	if (options.templateHelpers) {
		var customHelperPathPromises = resolveGlobs(options.templateHelpers);
		customHelperPathPromises.then(function(results) {
			_.each(results, function(val, index) {
				var helpersModulePath = process.env.PWD + '/' + val;
				var helpersWrapper = require(helpersModulePath);

				// Inject dependencies that can be used by the helpers
				var helpers = helpersWrapper(_, Handlebars);
				registerTemplateHelpers(helpers);
			});
		});
	}
	...
	return Promise.props({
		...
		return Promise.props({ templates : templates, schemas : schemas, pagedata: pageData })
			.bind(generator)
			.then(function (result) {
				// Make the page(s) and write pages to disk
				_.each(generator.makeHTML(result.templates, result.schemas, result.pagedata), _.partial(options.saveFiles || saveFiles, options.destination));
			}).catch(function (err) {
